        ...
        using (IServiceScope scope = _serviceScopeFactory.CreateScope())
        {
            ...
            foreach (var userId in _data.Keys)
            {
                ...
                if (userData.Choice == _currentQuestion!.RightAnswerIndex)
                {
                    ...
                    var goodAnswers = dbContext.Player.Where(p => p.UserId == userId).Single().NbRightAnswers++;
                    await dbContext.SaveChangesAsync();
                }
                else
                {
                    await _mathQuestionHub.Clients.User(userId).SendAsync("BadAnswer", _currentQuestion.RightAnswerIndex);
                }
